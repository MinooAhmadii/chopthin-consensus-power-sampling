#!/usr/bin/env python3
"""Check the HumanEval `cot` grading contract on real problems, offline (no model).

For each of the first N problems we synthesize the output the cot protocol expects (a
markdown block holding the canonical solution, stub included) and confirm that
  - CotProtocol.extract returns a standalone program, and
  - grading it with CotProtocol.grade_problem (prompt="") PASSES.
For information it also grades with the ORIGINAL problem, which makes the sandbox prepend
the stub a second time. Python tolerates the redefinition for well-formed programs, so this
usually passes too; the contract still uses prompt="" so the graded source is exactly the
extracted program.

Run from anywhere:  python tests/test_he_cot_contract.py   (also works under pytest)
"""
import json
import os
import sys
...
def run(n=N, verbose=True):
    ...
    for p in probs:
        ...
        code, path = cot.extract(response, p)
