...
def _chopthin_resample(w, eta, N, generator):
    """Torch wrapper around the pure-Python chopthin() resampler.

    Args:
        w:         [n] tensor of nonnegative weights (normalized or not) on some device.
        eta, N:    chopthin parameters (eta >= 4).
        generator: the resampling RNG stream (torch.Generator). A seed is drawn from it so
                   chopthin's internal systematic draws are reproducible and tied to it.
    Returns:
        (idx, new_w): idx = [N] LongTensor of ancestor indices on w.device;
                      new_w = [N] float tensor of the resampled (unequal) weights.
    """
    device = w.device
...
class SMCSamplingConfig:
    ...
    alpha_ramp_offset: int = 0  # v5: tokens already generated before this call; the
                                # internal step `t` is shifted by this offset when
                                # looking up alpha_t. Lets a chunked / island runner
                                # express one global ramp instead of one ramp per call.
    adaptive_temperature: bool = False  # v5: if True, the proposal at step t uses
                                        # τ_t = 1/alpha_t (i.e. proposal logits *= alpha_t).
                                        # This is Power-SMC Corollary 1's optimal per-step
                                        # proposal during the ramp. Default False matches
                                        # the prior baseline. Toggle on for paper-quality
                                        # runs; rerun the flat baseline with the same flag.
    ...
    # ---- Resampling scheme / trigger (chopthin experiments; P3) ----
    resampler: str = "systematic"    # {"systematic", "chopthin", "chopthin_reset"}
    trigger: str = "ess"             # {"ess", "robust_ratio", "always"}
    eta: float = 5.8284271247461903  # chopthin ratio bound 3+sqrt(8) (~0.5N ESS floor, large-n)
    robust_ratio_quantiles: Tuple[float, float] = (0.9, 0.1)  # (hi, lo) quantiles on log-weights
    robust_ratio_threshold: float = 2.0  # log-space (q_hi - q_lo) gap to trigger; CALIBRATE per P4
...
def smc_power_sample_memopt(
    ...
    # Two independent RNG streams: token sampling (g_sample) vs resampling/selection
    # (g_resample). Keeping them separate means the resampler's random draws never shift
    # the token-sampling stream, so token sampling stays identical across resampling
    # schemes (systematic vs chopthin) until particles actually diverge — required for
    # the paired A/B/C/D experiments. Seeded per call from the per-problem seed for
    # reproducibility; if seed is None, behavior is non-deterministic (legacy default).
    ...
    for t in range(Tmax):
        # ---- v5: alpha_t computed once at the TOP of the loop so it can drive ----
        # ---- both the proposal (when adaptive_temperature=True) and the weight ----
        # ---- update further down. The `+ alpha_ramp_offset` shift makes the    ----
        # ---- ramp global across chunked calls (see SMCSamplingConfig docstring). ----
        alpha_t = _alpha_ramp(
            t=t + int(cfg.alpha_ramp_offset),
        ...
        if cfg.adaptive_temperature:
            # τ_t = 1/alpha_t  ⇔  prop_logits = logits * alpha_t.
            # Power-SMC's per-step-optimal proposal — incremental weight is then
            # exactly Z_t(alpha_t) for every particle (deterministic given prefix).
            prop_logits_phys = last_logits.float() * float(alpha_t)
        elif cfg.temperature == 1.0:
        ...
        # ---- Alpha ramp + weight update ----
        # alpha_t was computed at the top of the loop; just use it here.
        cum_logp_prev = (
        ...
        if is_block_end:
            ...
            # ---- Resampling trigger (WHEN) ----
            if cfg.trigger == "ess":
                do_resample = ess < float(cfg.ess_threshold) * N
            elif cfg.trigger == "robust_ratio":
                q_hi = torch.quantile(lw, float(cfg.robust_ratio_quantiles[0]))
                q_lo = torch.quantile(lw, float(cfg.robust_ratio_quantiles[1]))
                rng_gap = float((q_hi - q_lo).item())
                stats.setdefault("robust_range_history", []).append(rng_gap)
                do_resample = rng_gap > float(cfg.robust_ratio_threshold)
            ...
            if do_resample:
                # ---- Resampling scheme (HOW) ----
                if cfg.resampler == "systematic":
                    idx_rs = systematic_resample(w, generator=g_resample)
                    post_log_w = None  # reset to uniform (log_w = 0)
                elif cfg.resampler in ("chopthin", "chopthin_reset"):
                    idx_rs, _new_w_ct = _chopthin_resample(
                        w, float(cfg.eta), N, g_resample
                    )
                    stats.setdefault("ess_after_history", []).append(
                        effective_sample_size(_new_w_ct / _new_w_ct.sum())
                    )
                    # "chopthin" keeps unequal weights; "chopthin_reset" resets to uniform
                    post_log_w = (
                        torch.log(_new_w_ct.clamp_min(1e-38))
                ...
                if post_log_w is None:
                    log_w = torch.zeros_like(log_w)           # systematic / chopthin_reset
                else:
                    log_w = post_log_w.to(log_w.dtype)         # chopthin: keep unequal weights
...
def smc_power_sample_multiround(
    ...
    for r in range(n_rounds):
        result = smc_power_sample_memopt(
            model, tokenizer, input_ids, round_cfg,
            seed=(int(seed) + r if seed is not None else None),
        )
